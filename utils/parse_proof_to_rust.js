...
export function parseProofToBytesArray(proof, compressed = false) {
  ...
  try {
    ...
    if (compressed) {
      ...
      const proofAIsPositive = yElementIsPositiveG1(BigInt(mydata.pi_a[1]))
        ? false
        : true;
      ...
      const proofBIsPositive = yElementIsPositiveG2(
        BigInt("0x" + Buffer.from(proofBY.slice(0, 32)).toString("hex")),
        BigInt("0x" + Buffer.from(proofBY.slice(32, 64)).toString("hex"))
      );
      proofB[0] = addBitmaskToByte(proofB[0], proofBIsPositive);
      const proofC = mydata.pi_c[0];
      const proofCIsPositive = yElementIsPositiveG1(BigInt(mydata.pi_c[1]));
