import {
    ...
    parseToBytesArray,
} from './generate_proof_and_public_signals.js'
...
For more information, try '--help'`
    ...
    const { proof, publicSignals } = await exportFullProof(proofInputs, wasmPath, zkeyPath)
...
pub fn verify_proof(data: &[u8]) -> Result<bool, Groth16Error> {
    ...
    let public_signals: [[u8; 32]; ${nPublic}] = chunk_instruction_data(&data[256..])
...
`
    ...
    const testRustProofAndPublicInput = `const PROOF: [u8; 128] = [${proofArr}];

const PUBLIC_SIGNALS: [[u8; 32]; ${nPublic}] = [${publicSignalsArr.map((arr) => `[${arr}]`)}];
const DATA: [u8; ${proofArr.length + publicSignalsArr.flat().length}] = [${[
        ...proofArr,
        ...publicSignalsArr.flat(),
    ]}];`
