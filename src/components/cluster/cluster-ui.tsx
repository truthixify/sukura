...
export function ClusterUiSelect() {
    ...
    return (
        <div className="dropdown dropdown-end">
            <label tabIndex={0} className="btn bg-white hover:bg-white rounded-btn text-black">
                {cluster.name}
            </label>
            <ul
                tabIndex={0}
            ...
            >
                {clusters.map((item) => (
                    <li key={item.name}>
                        <button
                            className={`btn btn-sm my-1 text-white hover:bg-base-100 ${item.active ? 'bg-base-100' : 'btn-ghost'}`}
