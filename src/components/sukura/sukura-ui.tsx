      ...
      {accounts.isLoading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {accounts.data?.map((account) => (
            <SukuraCard key={account.publicKey.toString()} account={account.publicKey} />
          ))}
        </div>
      ) : (
...
function SukuraCard({ account }: { account: PublicKey }) {
  const { accountQuery, incrementMutation, setMutation, decrementMutation, closeMutation } = useSukuraProgramAccount({
    account,
  })
