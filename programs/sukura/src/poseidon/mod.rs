...
impl Poseidon {
    ...
    pub fn hash_str(&self, inp: Vec<String>) -> Result<String, String> {
        let mut inp_fr: Vec<Fr> = Vec::new();
        for i in 0..inp.len() {
            let fr_el: Fr = Fr::from_str(&inp[i]).unwrap();
            inp_fr.push(fr_el);
        }
        Ok(self.hash(inp_fr)?.0.to_string())
    }
}
